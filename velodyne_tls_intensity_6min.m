...
% input file name
input_file_name = 'Scan4.pcap';

% output file name
output_file_name = 'Scan4_';

cd(input)

%% Initialization of parameters 

times = 3630; % Scan duration in tenths of a second (sec^-1) 
angle = 360; % LiDAR rotation angle (-360 if cable is up)
first = 20; % Time at the first frame

%% Export parameters
% Set to 0 to keep all the points
% Set to 1 to keep the x-positives values
% Set to 2 to keep the y-negatives values
pos = 2; % set to 1 normally / Set to 0 for calibration of a2

pos2 = 1; % set to 1 normally / Set to 0 to keep save the band 1 and 16 / Set to 1 to keep all the values Set to 2 to keep only band 8

gridStep = 0; %Set the grid resolution in [m] / Set to 0 to keep all the values

%% Calibration parameters
% Correction angle alpha_1 and alpha_2 [degrees]
% Those angle are determined automatically after calibration or be chosen

alpha_1 = 0; %incorrect value cause blur was 0.0277
alpha_2 = -1.1; %Incorrect value causes domeing
%alpha_3 = 1;
% Arm length [m]
R =     0;
